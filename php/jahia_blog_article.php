...
<html>
    <head>
         <meta charset="utf-8" />
         <title>Le site de Jahia - Articles du Blog</title>
    </head>
    <body>
    	 <div id="bloc_page">
    	 	<?php 
    	    ...
    	    include("jahia_nav.php"); 
    	 	...
    	 	{
    	 		echo '<p>' . $_GET['ok'] . '!!!!!!</p>';
    	 		echo '<div id="liens_blog"><img src="images/logo.png" alt="logo decoration" width="4%"><a href="jahia_blog.php">Retour a la liste des articles</a></div>';
    	 	}
			try
			{
			...
			$req2 = $bdd1->prepare('SELECT * FROM article WHERE id_article = ?');
			...
			$video = $donnees1['media_name'];
             ...
             if(preg_match("#^https#", $video))
            	...
            	{ ?>
                <h5><a href="jahia_blog_article.php?id_article=<?php echo $precedent ?>">Article precedent</a></h5><?php
            	} ?>
                <h5><a href="jahia_blog.php">Retour liste</a></h5><?php
                if (($_GET['id_article']) <= 8)
                { ?>
                  <h5><a href="jahia_blog_article.php?id_article=<?php echo $suivant ?>">Article suivant</a></h5>
                  </div><?php
                } ?> 
		    ...
		    <br>
			<h3><img src="images/logo.png" alt="logo decoration" width="4%">      COMMENTAIRES     <img src="images/logo.png" alt="logo decoration" width="4%"></h3>  
            <p>A Propos de l'article <?php echo $donnees1['titre_article']?></p>
            <?php
			$req2->closeCursor();

		     // afficher les commentaires
		    		// Récupération des 10 derniers messages
            		 $reponse = $bdd1->prepare('SELECT id_article, auteur, commentaire, DATE_FORMAT(date_commentaire, \'%d/%m/%Y à %Hh%imin%ss\') AS date_commentaire_fr FROM commentaires WHERE id_article = ? ORDER BY date_commentaire DESC');
            		 $reponse->execute(array($_GET['id_article']));

					// Affichage de chaque message (toutes les données sont protégées par htmlspecialchars)
					while ($donnees = $reponse->fetch())
					{
						echo '<span><h4> Commentaire de : <strong>' . htmlspecialchars($donnees['auteur']) . '</strong> posté le <em>' . htmlspecialchars($donnees['date_commentaire_fr']) . '</em></h4>';
           ...
           if (empty($_SESSION['pseudo']) AND empty($_COOKIE['pseudo']))
	           {	// on affiche la demande de l'auteur
				$req = $bdd1->prepare('SELECT * FROM article WHERE id_article = ?');
				$req->execute(array($id));
				// Affichage des données de l'article de l'id passé en GET
				$donnees2 = $req->fetch();
			    ?>
		            <h3><img src="images/logo.png" alt="logo decoration" width="4%">      AJOUTER UN NOUVEAU COMMENTAIRE     <img src="images/logo.png" alt="logo decoration" width="4%"></h3>  
		            <p>A Propos de l'article <?php echo $donnees2['titre_article']?></p>

		            <div style="text-align: center;">
		              <form action="jahia_blog_article_post.php?id_article=<?php echo $donnees1['id_article']?>" method="post">
		            	<div class="contacter">
				          <label>Auteur du commentaire :</label><br>
				          <input type="text" name="auteur" required placeholder="Ecrivez votre nom ici ... " />
				          <br>
				          <label>Commentaire :</label><br>
				          <textarea name="commentaire" required placeholder="Ecrivez votre commentaire ici ... "></textarea>
		                  <br>
				          </div>
			...
			elseif (!empty($_SESSION['pseudo'])) // si session ON
	            ...
	            <p>A Propos de l'article <?php echo $donnees2['titre_article']?></p>
	             ...
	             <div style="text-align: center;">
	              ...
	              <form action="jahia_blog_article_post.php?id_article=<?php echo $id; ?>" method="post">
	            	<div class="contacter">
			          <label><?php echo $_SESSION['pseudo']; ?> poste ton commentaire :</label><br>
			          <textarea name="commentaire" required placeholder="Ecris ton commentaire ici ... "></textarea>
	                  <br>
			          </div>
			          <input class="envoyer" type="submit" value="Envoyer" />
	              </form> 
	            ...
	            <div style="text-align: center;">
	              <form action="jahia_blog_article_post.php?id_article=<?php echo $id; ?>" method="post">
	            	<div class="contacter">
			          <label><?php echo $_COOKIE['pseudo']; ?> poste ton commentaire :</label><br>
			          <textarea name="commentaire" required placeholder="Ecris ton commentaire ici ... "></textarea>
	                  <br>
			          </div>
			          <input class="envoyer" type="submit" value="Envoyer" />
